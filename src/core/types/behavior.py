...
class BehaviorOutput:
    ...
    Invariantes:
      - `target_path.shape == (N+1, 3)`. Columnas: x, y, psi (rad).
        ...
        Coordenadas en frame ENU del mapa BFMC (mismas que GraphML).
      - `speed_profile.shape == (N,)`. Velocidad de referencia para los
        N pasos de control. Unidades m/s. Valores >= 0; el `velocity_overlay`
        se encarga de capear y aplicar zeros cuando hay stoplines.
      ...
      - `scenario_name` es el escenario que generó el plan (antes del
        velocity_overlay). El dashboard lo muestra para diagnóstico.
