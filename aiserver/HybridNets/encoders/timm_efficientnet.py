...
try:
    ...
    try:
        ...
        # Swish es equivalente a SiLU en PyTorch
        Swish = torch.nn.SiLU
...
def prepare_settings(settings):
    # Valores por defecto de ImageNet como fallback seguro
    _defaults = {
    ...
    # timm viejo: dict plano
    if isinstance(settings, dict):
        return {
            "mean": settings.get("mean", _defaults["mean"]),
            "std": settings.get("std", _defaults["std"]),
            "url": settings.get("url", _defaults["url"]),
            "input_range": (0, 1),
            "input_space": "RGB",
        }
    # timm >= 0.9: DefaultCfg / PretrainedCfg — proteger contra cualquier excepción
    try:
        cfg = settings
        try:
            cfg = settings.default  # puede lanzar IndexError si tags está vacío
        except Exception:
            pass
        return {
            "mean": getattr(cfg, 'mean', _defaults["mean"]),
            "std": getattr(cfg, 'std', _defaults["std"]),
            "url": getattr(cfg, 'url', _defaults["url"]),
            "input_range": (0, 1),
            "input_space": "RGB",
        }
    except Exception:
